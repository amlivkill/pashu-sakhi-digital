...
function navigate(route) {
    ...
    if (route === 'dashboard') {
        ...
        } else if (state.user.role === 'block') {
            viewHTML = window.BlockDashboard(state.user, navigate);
            bottomNav.classList.add('hidden'); // Block has no bottom nav
        } else {
            // Default Sakhi
    ...
    setTimeout(() => {
        if (!viewHTML) {
            viewHTML = '<div class="p-4">Error loading view. <button onclick="location.reload()">Reload</button></div>';
        }
...
// Debug mode: Log all local storage on init
console.log("App Initialized. State:", state);
if (location.search.includes('reset')) {
    localStorage.clear();
    console.log("Resetting via URL...");
}
