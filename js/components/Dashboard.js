window.Dashboard = function (user, navigateTo) {
    ...
    <header class="flex justify-between items-center mb-4 fade-in">
        ...
        <div class="flex gap-2">
            <button onclick="window.utils.shareApp()" style="background: white; padding: 0.5rem; border-radius: 50%; box-shadow: var(--shadow-sm); border: none; cursor: pointer;">
                🔗
            </button>
    ...
    setTimeout(() => {
        ...
        console.log("Dashboard: Found activity buttons count:", btns.length);
        if (btns.length === 0) {
            console.error("Dashboard: NO BUTTONS FOUND! Check if innerHTML was updated.");
        }
        btns.forEach(btn => {
            console.log("Dashboard: Attaching listener to:", btn.dataset.route);
            btn.addEventListener('click', () => {
                const route = btn.dataset.route;
                console.log("Dashboard: Button clicked route:", route);
                alert("Button Clicked: " + route); // Debug alert
                navigateTo(route);
            });
        });
    }, 200);
