window.ActivityForm = function (type, navigateTo) {
    ...
    const formConfig = {
        ...
        'insurance': {
            ...
            fields: [
                ...
                { name: 'village', label: window.t('lbl_village'), type: 'text', placeholder: window.t('lbl_village_hint'), required: true },
                { name: 'beneficiary', label: window.t('lbl_beneficiary'), type: 'text', required: true },
                { name: 'animal_type', label: window.t('lbl_animal'), type: 'select', options: ['Cow / गाय', 'Buffalo / भैंस', 'Goat / बकरी', 'Mule / खच्चर'] },
    ...
    setTimeout(() => {
        ...
        document.getElementById('generic-form').addEventListener('submit', async (e) => {
            ...
            // 1. Geofencing Check
            if (user && user.block) {
                const center = window.MasterData.blockCenters[user.block];
                if (center && locationData) {
                    const distance = window.GPS.calculateDistance(
                        locationData.lat, locationData.lng,
                        center.lat, center.lng
                    );
                    if (distance > 50) {
                        const proceed = confirm(`${window.t('alert_geofence_title')}\n\n${window.t('alert_geofence_msg')}`);
                        if (!proceed) return;
                    }
                }
            }
            ...
            if (window.DB) {
                ...
                const isDuplicate = entries.some(entry => {
                    ...
                    return entry.data.activityType === type &&
                        entry.data.beneficiary === data.beneficiary &&
                        entryDate === today;
                });

                if (isDuplicate) {
                    const proceed = confirm(`${window.t('alert_duplicate_title')}\n\n${window.t('alert_duplicate_msg')}`);
                    if (!proceed) return;
                }
            }
